...
categorize_wallpaper() {
    ...
    # Exit early if AI styling is disabled or the payload file doesn't exist
    if [[ ! -n "$ai_script" || ! -f "$target_payload" ]]; then
        return
    fi
    ...
    if [[ -f "$cache_file" ]]; then
        # Cache Hit: Fast pathway, skip all loop index calculations
        (
            sleep 0.2
            cat "$cache_file" >"$STATE_DIR/user/generated/wallpaper/category.txt"
        ) &
    else
        # Cache Miss: Validate output cleanly strictly on ingestion
        ...
        (
            ...
            # Query the backend engine into isolation workspace
            "$ai_script" "$target_payload" >"$tmp_output" 2>"$STATE_DIR/user/generated/wallpaper/ai_error.log"

            # Clean and match response structure against expected keywords
            ...
            # Commit persistently only if data structure passed tests
    ...
    if [[ "$color_flag" == "1" ]]; then
        ...
        if is_video "$imgpath"; then
            ...
            if [ -f "$thumbnail" ]; then
                ...
                # Pass the extracted static frame path
            ...
            # Pass the original static image path directly
